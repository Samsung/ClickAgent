...
from api_model import inference_chat
from chat_internvl import add_response, init_action_chat, init_eval_chat
from controller import AndroidDevice, AndroidEmulatorException, AdbException
from prompt import get_action_prompt, build_final_eval_v3_final_prompt_web
from utils import draw_click, draw_rectangle
...
def run(config, and_device):
    ...
    for iteration in range(int(config.other.max_steps)):
        ...
        chat_action = add_response("user", prompt_action, chat_action, screenshots[-1])
        print("Querying LLM #1")
        output_action = inference_chat(chat_action, model_name, API_URL, token)
        print("Finished LLM Querying #1")
        ...
        prompt_eval = build_final_eval_v3_final_prompt_web(instruction, action_history)
        chat_eval = init_eval_chat()
        chat_eval = add_response("user", prompt_eval, chat_eval)
        print("Querying LLM #2")
        output_eval = inference_chat(chat_eval, model_name, API_URL, token)
        print("Finished LLM Querying #2")
        reflect_done = re.search(r"Status:(.*)\n", output_eval)
        if reflect_done:
            reflect_done = reflect_done.group(1).strip()

        # Action completed successfully. Exit
        if "fail" not in reflect_done.lower():
            break
        ...
        trajectory_log["steps"].append({
            ...
            "other": {
                ...
                "reflection_status": reflect_done
            }
        })
